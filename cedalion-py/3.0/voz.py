...
class InterfaceVoz:
    def __init__(self):
        ...
        # Guarda o ID da voz em português para reaplicar depois
        self.voice_id = None
        vozes = self.engine.getProperty('voices')
        for voz in vozes:
            if 'brazil' in voz.name.lower() or 'pt-br' in voz.id.lower() or 'portuguese' in voz.name.lower():
                self.voice_id = voz.id
                self.engine.setProperty('voice', self.voice_id)
                break
        # Se não achar voz PT, mantém a padrão mas guarda o ID atual
        if self.voice_id is None and vozes:
            self.voice_id = vozes[0].id
    ...
    def falar(self, texto):
        """Fala o texto fornecido, reinicializando o engine a cada chamada para evitar conflitos com pyaudio."""
        print(f"\n[Cedalión]: {texto}")
        
        # Para o engine atual para liberar recursos de áudio
        try:
            self.engine.stop()
        except:
            pass
        
        # Reinicializa o engine do zero
        self.engine = pyttsx3.init()
        
        # Reaplica as configurações salvas
        self.engine.setProperty('rate', self.rate)
